  ...
  return (dispatch) => {
    ...
    const signInGmail = async () => {
      try {
        ...
        // refrence to user data on fire base store
        const userDataRef = doc(db, "UsersData", signInRes?.user?.uid);
        // check user cart data
        const userDataResponse = await getDoc(userDataRef);
        const userData = userDataResponse.data((d) => d);
        // if user have not any data on database set a new cell
        if (!userData) {
          // user collection sample :
          const newUserCollection = {
          ...
          // create new user data collection with (User Uid)
          await setDoc(userDataRef, newUserCollection);
        }
        // disptach success after requests
        dispatch({
          type: AUTH_USER_SUCCESS,
